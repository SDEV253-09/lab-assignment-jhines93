...
function change_password($username, $old_password, $new_password) {
    ...
    $conn = db_connect();
    $result = $conn->query("update user set passwd = sha1('".$new_password."')'".$username."'"); if (!$result) { throw new Exception('Password could not be changed.');
    } else {
        return true;
...
function notify_password($username, $password) {
    ...
    if (!$result) {
        ...
        $email = $row->email; $from = "From: support@phpbookmark \r\n";
